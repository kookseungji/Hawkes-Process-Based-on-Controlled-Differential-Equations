...
class _VectorField_augment(torch.nn.Module):
    def __init__(self, X, func, ode_func, event_time, is_tensor, is_prod):
        super(_VectorField_augment, self).__init__()
    ...
    def forward(self, t, z):
        ...
        h , ode_z = z[0] , z[1]
        # control_gradient is of shape (..., input_channels)
        control_gradient = self.X.derivative(t) # self.X.weight(t)
        
        if self.is_prod:
            # out is of shape (..., hidden_channels)
            out = self.func.prod(t, h, control_gradient)
        else:
            # vector_field is of shape (..., hidden_channels, input_channels)
            vector_field = self.func(t, h) 
            # f = self.func(t, self.ode_func(t,self.event_time, z))
            # g = self.ode_func(t, ode_z)
            # g = self.ode_func(t, self.add_func(t, ode_z))

            if self.is_tensor:
                # out is of shape (..., hidden_channels)
                # (The squeezing is necessary to make the matrix-multiply properly batch in all cases)
                f = (vector_field @ control_gradient.unsqueeze(-1)).squeeze(-1)
                # g = self.ode_func(t, f)
            else:
                f = tuple((vector_field_ @ control_gradient_.unsqueeze(-1)).squeeze(-1)
                            for vector_field_, control_gradient_ in zip(vector_field, control_gradient))
...
def cdeint_augment(X, func, z0, a0, ode_func, event_time, t, adjoint=True, backend="torchdiffeq", **kwargs):
    r"""Solves a system of controlled differential equations.
        ...
        for buffer in X.buffers():
            ...
            if buffer.requires_grad:
                warnings.warn("One of the inputs to the control path X requires gradients but "
                              ...
                              "cdeint(X=X, func=func, ..., adjoint_params=adjoint_params)\n"
                              "```")
    vector_field = _VectorField_augment(X=X, func=func, ode_func= ode_func, event_time = event_time, is_tensor=is_tensor, is_prod=is_prod)
    ...
    if is_tensor:
        batch_dims = range(1, len(out.shape) - 1)
        out = out.permute(*batch_dims, 0, -1)
        batch_dims = range(1, len(ode_out.shape) - 1)
        ode_out = ode_out.permute(*batch_dims, 0, -1)
    else:
        out_ = []
        for outi in out:
            batch_dims = range(1, len(outi.shape) - 1)
...
class _VectorField_augment2(torch.nn.Module):
    def __init__(self, X, func, ode_func, event_time, is_tensor, is_prod):
        super(_VectorField_augment2, self).__init__()
...
def cdeint_augment2(X, func, z0, a0, ode_func, event_time, t, adjoint=True, backend="torchdiffeq", **kwargs):
    r"""Solves a system of controlled differential equations.
        ...
        for buffer in X.buffers():
            ...
            if buffer.requires_grad:
                warnings.warn("One of the inputs to the control path X requires gradients but "
                              ...
                              "cdeint(X=X, func=func, ..., adjoint_params=adjoint_params)\n"
                              "```")
    vector_field = _VectorField_augment2(X=X, func=func, ode_func= ode_func, event_time = event_time, is_tensor=is_tensor, is_prod=is_prod)
